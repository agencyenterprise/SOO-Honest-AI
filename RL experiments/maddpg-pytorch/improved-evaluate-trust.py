...
def run_experiment(config, model_num):
    ...
    for ep_i in range(config.n_episodes):
        ...
        for t_i in range(config.episode_length):
            ...
            #print("actions: ", actions, " actions[0]: ", actions[0], "pretrained actions: ", pretrained_agent_actions, " pretrained [0]: ", pretrained_agent_actions[0][0])
            actions[0] = pretrained_agent_actions[0][0]
...
def run(config):
    ...
    for seed in seeds:
            ...
            print("\nLoading deceptive baseline: 297")
            deceptive_normal_rew, deceptive_adv_rew = run_experiment(config, 297)
            deceptive_total_normal_rew.append(deceptive_normal_rew)
            deceptive_total_adv_rew.append(deceptive_adv_rew)

            print("\nLoading non-deceptive baseline: 98")
            non_deceptive_normal_rew, non_deceptive_adv_rew  = run_experiment(config, 98)
    ...
    fig, ax = plt.subplots()
    bars1 = ax.bar(x - width/2, deceptive_means, width, label='Deceptive', yerr=deceptive_stds, capsize=10)
    bars2 = ax.bar(x + width/2, non_deceptive_means, width, label='Non-Deceptive', yerr=non_deceptive_stds, capsize=10)

    # Adding labels, title, and legend
    ax.set_ylabel('Mean Reward')
    ax.set_title('Mean Reward Comparison of Baselines with Trusting Adversary')
    ax.set_xticks(x)
