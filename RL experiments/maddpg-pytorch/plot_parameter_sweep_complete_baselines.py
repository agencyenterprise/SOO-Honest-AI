...
def save_complete_data(run_num, complete_data):
    complete_file = f'/Users/marc/Desktop/Work/starter-codebase/maddpg-pytorch/benchmarks/agent/run{run_num}_complete.npy'
...
def create_visualization_and_save(runs, seeds):
    ...
    all_data1, all_data2 = aggregate_data_for_runs(runs, seeds)

    # Save the complete data
    save_complete_data(runs[0], all_data1)
    save_complete_data(runs[1], all_data2)
    ...
    Z1_mean = np.mean(all_data1, axis=0)
    Z1_std = np.std(all_data1, axis=0)
    Z2_mean = np.mean(all_data2, axis=0)
    Z2_std = np.std(all_data2, axis=0)
